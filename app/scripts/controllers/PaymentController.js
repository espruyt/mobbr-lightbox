angular.module('mobbr-lightbox.controllers')
    .controller('PaymentController', function ($scope, $location, $routeParams, MobbrPayment, MobbrPerson, MobbrBalance, MobbrUri, MobbrUser, mobbrSession) {
        'use strict';
        ...
        var url = window.atob($routeParams.hash);
        ...
        $scope.preview = function (showPreview, callBack) {
            ...
            if ($scope.form.amount && currency) {
                $scope.previewLoading = MobbrPayment.preview({
                    data: $scope.url,
                    currency: currency,
                    amount: $scope.form.amount,
                    invoiced: $scope.form.wantInvoices
                }, function (response) {
                    if (callBack) {
        ...
        $scope.login = function () {
            $scope.authenticating = MobbrUser.passwordLogin({ username: $scope.loginform.username, password: $scope.loginform.password }, handleMessage, handleMessage);
        };
